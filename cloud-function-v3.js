// cloud-function-v3.js - 修复 Content-Type 问题
    ...
    try {
        ...
        if (!fs.existsSync(fullPath)) {
            ...
            return {
                ...
                body: '<h1>404 - File not found</h1><p>文件不存在：' + filePath + '</p>'
            };
        }
        ...
        // 判断是否为文本文件
        const isTextFile = ['html', 'css', 'js', 'json', 'xml', 'txt'].includes(path.extname(filePath).slice(1));
        
        // 腾讯云 SCF 事件函数响应格式 - 文本文件直接返回字符串，避免被腾讯云包装
        if (isTextFile) {
            return {
                statusCode: 200,
                headers: { 
                ...
                body: contentBuffer.toString('utf8')
            };
        } else {
            // 二进制文件（图片等）使用 Base64
            return {
        ...
        return {
            ...
            body: '<h1>500 - Internal Server Error</h1><p>' + e.message + '</p>'
        };
    }
}
